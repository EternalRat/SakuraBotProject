...
module.exports.run = async (bot, message, args) => {
    ...
    var missingArgsEmbed = new Discord.RichEmbed() // Creates the embed thats sent if the command isnt run right
        ...
        .setDescription('Usage: `' + prefix + 'deletewarn [@User] [number]')
        .setTimestamp();
    ...
    let ndw = args.slice(1).join(' ') // .slice(1) removes the user mention, .join(' ') joins all the words in the message, instead of just sending 1 word
    if (!ndw) return message.channel.send(missingArgsEmbed);

    if (!warns[mentioned.id]) warns[mentioned.id] = {
        warns: 0
    }
    warns[mentioned.id].warns = warns - ndw;
    ...
    var dwEmbed = new Discord.RichEmbed()
        ...
        .addField(`Number of warning deleted`, ndw)
        .setTimestamp();
